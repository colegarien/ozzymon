...
namespace Ozzymon
{
    ...
    {
        // this will be the ozzymon the player is holding
        List<Ozzymon> belt = new List<Ozzymon>();
        ...
        // the player starts from nowhere
        string whereFrom = "";
        ...
        public Vector2 getTargetPos() { return target; }
        ...
        // gets where player is from
        public string getWhereFrom()
        {
            return whereFrom;
        }

        // sets where the player is from
        public void setWhereFrom(string s)
        {
            whereFrom = s;
        }
        ...
        public void Update(GameTime gameTime, Area a, List<NPC> npcList, List<int> curNpcs)
        {
            ...
            if (CanMoveUp(a.getLayout(), a.getWallLayout()) && notNPC(new Vector2(0, -1), npcList, curNpcs))
            {
                MoveUp();
            }
            else if (!actioning && !walking && (Keyboard.GetState().IsKeyDown(Keys.W) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.Y > .1)) { direction = 0; }
            else if (CanMoveDown(a.getLayout(), a.getWallLayout(), a.getHeight()) && notNPC(new Vector2(0, 1), npcList, curNpcs))
            {
                MoveDown();
            }
            else if (!actioning && !walking && (Keyboard.GetState().IsKeyDown(Keys.S) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.Y < -.1)) { direction = 2; }
            else if (CanMoveLeft(a.getLayout(), a.getWallLayout()) && notNPC(new Vector2(-1, 0), npcList, curNpcs))
            {
                MoveLeft();
            }
            else if (!actioning && !walking && (Keyboard.GetState().IsKeyDown(Keys.A) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.X < -.1)) { direction = 3; }
            else if (CanMoveRight(a.getLayout(), a.getWallLayout(), a.getWidth()) && notNPC(new Vector2(1, 0), npcList, curNpcs))
            {
                MoveRight();
            }
            else if (!actioning && !walking && (Keyboard.GetState().IsKeyDown(Keys.D) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.X > .1)) { direction = 1; }
            else if (CanAction())
            {
                DoAction();
            }
        ...
        private bool notNPC(Vector2 nextLoc, List<NPC> npcList, List<int> curNpcs)
        {
            ...
            {
                if (npcList[curNpcs[i]].getX() == pos.X + nextLoc.X && npcList[curNpcs[i]].getY() == pos.Y + nextLoc.Y)
                    return false;
            }
        ...
        private bool CanAction()
        {
            ...
            bool b = !actioning && !walking && (Keyboard.GetState().IsKeyDown(Keys.Space) || ButtonState.Pressed == GamePad.GetState(PlayerIndex.One).Buttons.A);
            return b;
        }
        ...
        private bool CanMoveUp(int[,] l, Tile[,] walls)
        {
            bool r = (!actioning && !walking && pos.Y - 1 >= 0 && !(l[(int)pos.X, (int)pos.Y - 1] == 1 && (walls[(int)pos.X, (int)pos.Y - 1].isAWall() || walls[(int)pos.X, (int)pos.Y - 1].isAWater())) && (Keyboard.GetState().IsKeyDown(Keys.W) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.Y > .1));
        ...
        private bool CanMoveDown(int[,] l, Tile[,] walls, int h)
        {
            bool r = (!actioning && !walking && pos.Y + 1 < h && !(l[(int)pos.X, (int)pos.Y + 1] == 1 && (walls[(int)pos.X, (int)pos.Y + 1].isAWall() || walls[(int)pos.X, (int)pos.Y + 1].isAWater())) && (Keyboard.GetState().IsKeyDown(Keys.S) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.Y < -0.1));
        ...
        private bool CanMoveLeft(int[,] l, Tile[,] walls)
        {
            bool r = (!actioning && !walking && pos.X - 1 >= 0 && !(l[(int)pos.X - 1, (int)pos.Y] == 1 && (walls[(int)pos.X - 1, (int)pos.Y].isAWall() || walls[(int)pos.X - 1, (int)pos.Y].isAWater())) && (Keyboard.GetState().IsKeyDown(Keys.A) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.X < -.1));
        ...
        private bool CanMoveRight(int[,] l, Tile[,] walls, int w)
        {
            bool r = (!actioning && !walking && pos.X + 1 < w && !(l[(int)pos.X + 1, (int)pos.Y] == 1 && (walls[(int)pos.X + 1, (int)pos.Y].isAWall() || walls[(int)pos.X + 1, (int)pos.Y].isAWater())) && (Keyboard.GetState().IsKeyDown(Keys.D) || GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.X > .1));
